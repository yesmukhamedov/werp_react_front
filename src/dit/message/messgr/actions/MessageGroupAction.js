...
/* action URLs */
const messageGroupUrl = `core/messgr`;
...
export function removeMessageGroup(groupId, successCallback) {
    ...
    return dispatch => {
        ...
        doDelete(`${messageGroupUrl}/${groupId}`)
            .then(() => {
                dispatch(
                    notify('success', 'Message group was deleted.', 'Успешно'),
                );
                if (successCallback) {
                    successCallback();
                }
...
export function createMessageGroup(groupName, successCallback) {
    ...
    return dispatch => {
        ...
        doPost(messageGroupUrl, groupName)
            .then(() => {
                dispatch(
                    notify('success', 'Message group was created.', 'Успешно'),
                );
                if (successCallback) {
                    successCallback();
                }
...
export function updateMessageGroup(groupId, groupName, successCallback) {
    ...
    return dispatch => {
        ...
        doPut(`${messageGroupUrl}/${groupId}`, groupName)
            .then(() => {
                dispatch(
                    notify('success', 'Message group was updated.', 'Успешно'),
                );
                if (successCallback) {
                    successCallback();
                }
