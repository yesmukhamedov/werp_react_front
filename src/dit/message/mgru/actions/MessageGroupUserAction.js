...
export function createMessageGroupUser(formValues, successCallback) {
...
export function fetchReferences(lang) {
  return (dispatch) => {
    axios
      .all([GET(departmentsUrl), GET(usersUrl), GET(messageGroupUrl), GET(taskAdminUrl)])
      .then(axios.spread((
        { data: deptList }, { data: userList }, { data: messageGroupList }, { data: taskAdminList }
      ) => {
