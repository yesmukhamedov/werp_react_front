    ...
    if (a_ubObject.userBranchId) {
      return (
        <Icon
          ...
          onClick={() => this.onRowSelect(a_ubObject)}
        />
      );
    }
  }
      ...
      return this.props.userBranchList.map((ub, idx) => (
        <Table.Row key={ub.branchId}>
          ...
          <Table.Cell>
            <input
              type="checkbox"
              checked={ub.flagExists}
              name={idx}
              className="checkBox"
              onChange={event => this.handleChangeCheckbox(event, idx)}
            />
          </Table.Cell>
      ...
      <div id="bukrsBranchDiv">
        <div>
          <Button
            ...
            size="small"
            onClick={() =>
              this.props.editUserBranches(
                this.props.selectdeUser.userId,
                this.props.userBranchList,
              )
            }
          >
        ...
        <Table striped compact collapsing id="bukrsBranchTable">
          ...
          <Table.Body>{this.renderBukrsBranchList()}</Table.Body>
        </Table>
      </div>
    );
  }
...
export default connect(
  mapStateToProps,
  { markBranch, editUserBranches, fethcUserBranchCustomers },
)(BukrsBranchList);
