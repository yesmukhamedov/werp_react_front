...
const PopupInfo = (belnr,gjahr,dmbtr,wrbtr,dmbtr_paid,wrbtr_paid,iinBin,lifnrName,lifnr,waers) => (
    <Popup trigger={ <span><Icon name='info' /></span>} flowing hoverable>
       <Table compact celled>
            ...
            <Table.Body>
                <Table.Row>
                    <Table.Cell collapsing><p>{belnr}</p></Table.Cell>
                    <Table.Cell collapsing><p>{gjahr}</p></Table.Cell>
                    <Table.Cell collapsing><p>{moneyFormat(dmbtr)} USD</p></Table.Cell>
                    <Table.Cell collapsing><p>{moneyFormat(wrbtr)} {waers}</p></Table.Cell>
...
class Arli extends Component {
    ...
    constructor(props){
        ...
        this.state={searchTerm:{bukrs:'',branchList:[],dateFrom:moment(firstDay),dateTo:moment(lastDay)}, activeIndex:0};
    ...
    searchResult(){
        ...
        this.props.fetchARLI(bukrs,branchList, moment(dateFrom).format("YYYY-MM-DD"), moment(dateTo).format("YYYY-MM-DD")
        ,(val)=>{this.setState({activeIndex:val})});
    }
                            ...
                            <Table.Body>
                                <Table.Row>
                                    ...
                                    <Table.Cell colSpan="2">
                                        <Dropdown fluid placeholder='Компания' selection options={companyOptions} value={bukrs} 
                                ...
                                <Table.Row>
                                    ...
                                    <Table.Cell colSpan="2">
                                        <Dropdown placeholder='Все' fluid multiple search selection options={bukrs?branchOptions[bukrs]:[]} 
                                            value={branchList}  onChange={(e,{ value }) => this.onInputChange(value,'branchList')} />                                        
                                    </Table.Cell>
                                </Table.Row>
                                <Table.Row>
                                    <Table.Cell>                                        
                                ...
                                <Table.Row>
                                    ...
                                    <Table.Cell>
                                        <Button icon labelPosition='left' primary size='small' onClick={()=>this.searchResult()}>
    ...
    render(){
        ...
        let t1columns = [];
        let t1r1c1 = {Header:({value}) => <b>Филиал</b>,accessor: "branchName",
        ...
        let t1r1c2 = {Header:({value}) => <b>Фин. док и Рег. номер</b>,accessor: "zreg",Cell: obj => (<span> <Link target='_blank' className={'ui icon button'} 
                    ...
                    {PopupInfo(
                        ...
                        obj.original.dmbtr_paid, obj.original.wrbtr_paid, obj.original.iinBin,
                        obj.original.lifnrName, obj.original.lifnr, obj.original.waers

                    )}
                    </span>), width: 220};
                    
                    t1r1c2.Footer = (<span><strong>Кол. Счет Фактур: {this.props.numinv}</strong></span>);
        let t1r1c3 = {Header:({value}) => <b>ФИО</b>,accessor: "lifnrName",
                                            filterMethod: (filter, rows) =>
                                                matchSorter(rows, filter.value, { keys: ["lifnrName"] }),
                                            filterAll: true
                    };
        let t1r1c4 = {Header:({value}) => <b>Дата проводки</b>,accessor: "budatYYYYMMDD",Cell: obj => <span>{obj.original.budat}</span>,
        ...
        let t1r1c6 = {Header:({value}) => <b>Название</b>,accessor: "matnrName",
                                            filterMethod: (filter, rows) =>
                                                matchSorter(rows, filter.value, { keys: ["matnrName"] }),
                                            filterAll: true};
        let t1r1c7 = {Header:({value}) => <b>Кол.</b>,accessor: "menge", width: 80};
            t1r1c7.Footer = (<span><strong>{moneyFormat(_.sum(_.map(this.props.table, d => d.menge)))}</strong></span>);

        let t1r1c8 = {Header:({value}) => <b>Сумма ВВ</b>,accessor: "dmbtr_p",Cell: ({value}) => (<span>{moneyFormat(value)} USD</span>), width: 150};        
            t1r1c8.Footer = (<span><strong>{moneyFormat(new BigNumber(_.sum(_.map(this.props.table, d => d.dmbtr_p)).toFixed(2)))}</strong></span>); 

        let t1r1c9 = {Header:({value}) => <b>Сумма В</b>,accessor: "wrbtr_p",Cell: obj => (<span> {moneyFormat(obj.value)} {obj.original.waers}</span>)
                                        , width: 150};        
            t1r1c9.Footer = (<span><strong>{moneyFormat(_.sum(_.map(this.props.table, d => d.wrbtr_p)))}</strong></span>);    
        ...
        return (
            ...
            <Container fluid style={{ marginTop: '2em', marginBottom: '2em', paddingLeft: '2em', paddingRight: '2em'}}>
                ...
                <Menu pointing stackable>
                    <Menu.Item name='Параметры поиска' active={activeIndex === 0}  onClick={()=>{this.onInputChange(0,'activeIndex')}} />
                    <Menu.Item name='Результат' active={activeIndex === 1} onClick={()=>{this.onInputChange(1,'activeIndex')}} />
