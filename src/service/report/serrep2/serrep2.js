...
class Serrep2 extends Component {
  ...
  onSearchClick() {
    ...
    let searchDate = '';
    if (
      this.state.searchTerm.archive &&
      (this.state.searchTerm.date === null ||
        this.state.searchTerm.date.length === 0)
    ) {
  ...
  onSelectTableType(index) {
    if (index === 1) {
      this.setState({
        ...this.state,
        currentTable: this.state.tek,
        button1: true,
        button2: false,
        button3: false,
        currentTableTotal: this.state.tekTotal,
      });
    } else if (index === 2) {
      this.setState({
        ...this.state,
        currentTable: this.state.pros,
        button1: false,
        button2: true,
        button3: false,
        currentTableTotal: this.state.prosTotal,
      });
    } else if (index === 3) {
      this.setState({
        ...this.state,
        currentTable: this.state.all,
        button1: false,
        button2: false,
        button3: true,
        currentTableTotal: this.state.allTotal,
      });
    }
  }
  ...
  renderTableFooter() {
    return this.state.currentTableTotal.map((wa, idx) => (
      <Table.Row key={idx}>
        ...
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.f1)}</b>
        </Table.HeaderCell>
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.f2)}</b>
        </Table.HeaderCell>
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.f3)}</b>
        </Table.HeaderCell>
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.f4)}</b>
        </Table.HeaderCell>
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.f5)}</b>
        </Table.HeaderCell>
        <Table.HeaderCell>
          <Label>{wa.waers}</Label>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.dmbtr)}</b>
          {/* <NumberFormat thousandSeparator={' '}  value={wa.dmbtr} decimalScale={2} fixedDecimalScale/> */}
        </Table.HeaderCell>
      </Table.Row>
    ));
            ...
            <Grid.Column mobile={16} tablet={16} computer={4}>
              <Grid verticalAlign="middle" textAlign="justified">
                ...
                <Grid.Row columns={2}>
                  ...
                  <Grid.Column mobile={16} tablet={8} computer={12}>
                    <Checkbox
                      checked={this.state.searchTerm.today}
                      onChange={event => this.onInputChange(event, 'today')}
                    />
                  </Grid.Column>
                </Grid.Row>
                <Grid.Row columns={2}>
                  ...
                  <Grid.Column mobile={16} tablet={8} computer={12}>
                    <Checkbox
                      checked={this.state.searchTerm.archive}
                      onChange={event => this.onInputChange(event, 'archive')}
                    />
                  </Grid.Column>
                </Grid.Row>
                <Grid.Row columns={2}>
                  ...
                  <Grid.Column mobile={16} tablet={8} computer={12}>
                    <DatePicker
                      ...
                      disabled={!this.state.searchTerm.archive}
                      dateFormat="MM.YYYY"
                    />
                  </Grid.Column>
                </Grid.Row>
