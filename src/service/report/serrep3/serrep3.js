...
class Serrep3 extends Component {
  constructor(props) {
    ...
    this.state = {
      searchTerm: { bukrs: '', branchList: [], date: moment() },
      companyOptions: [],
      branchOptions: [],
      button1: true,
      button2: false,
  ...
  renderTable() {
    return this.state.currentTable.map((wa, idx) => (
      <Table.Row key={idx}>
        ...
        <Table.Cell>
          {new Intl.NumberFormat('ru-RU').format(wa.total)}
        </Table.Cell>
        <Table.Cell>{new Intl.NumberFormat('ru-RU').format(wa.war)}</Table.Cell>
        <Table.Cell>
          {new Intl.NumberFormat('ru-RU').format(wa.notWar)}
        </Table.Cell>
      </Table.Row>
    ));
  }
  ...
  renderTableFooter() {
    return this.state.currentTableTotal.map((wa, idx) => (
      <Table.Row key={idx}>
        ...
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.total)}</b>
        </Table.HeaderCell>
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.war)}</b>
        </Table.HeaderCell>
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.notWar)}</b>
        </Table.HeaderCell>
      </Table.Row>
    ));
  }
