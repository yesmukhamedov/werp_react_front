...
class Serrep1 extends Component {
  ...
  onSearchClick() {
    ...
    if (
      this.state.searchTerm.bukrs === null ||
      this.state.searchTerm.bukrs.length === 0
    ) {
    ...
    if (
      this.state.branchOptions === null ||
      this.state.branchOptions.length === 0
    ) {
      this.props.notify(
        'error',
        'Обратитесь к администратору, вам не присвоены филиалы',
        'Ошибка',
      );
      return;
    }
    ...
    if (
      this.state.searchTerm.dateFrom === null ||
      this.state.searchTerm.dateFrom.length === 0
    ) {
    ...
    if (
      this.state.searchTerm.dateTo === null ||
      this.state.searchTerm.dateTo.length === 0
    ) {
      this.props.notify('error', 'Выберите период', 'Ошибка');
      return;
    }
    let strVal = this.state.searchTerm.dateFrom.format('YYYY-MM-DD');
    const searchDateFrom = moment.utc(strVal).format();
    strVal = this.state.searchTerm.dateTo.format('YYYY-MM-DD');
    const searchDateTo = moment.utc(strVal).format();

    axios
      .get(`${ROOT_URL}/api/service/reports/serrep1/search`, {
        headers: {
  ...
  onSelectTableType(index) {
    if (index === 1) {
      this.setState({
        ...this.state,
        currentTable: this.state.pyl,
        button1: true,
        button2: false,
        button3: false,
        currentTableTotal: this.state.pylTotal,
        displayTable: 'hide',
        displayColumn: 'hide',
      });
    } else if (index === 2) {
      ...
      this.setState({
        ...this.state,
        currentTable: this.state.filter,
        button1: false,
        button2: false,
        button3: true,
        currentTableTotal: this.state.filterTotal,
        displayTable: 'show',
        displayColumn: 'show',
      });
    }
  ...
  renderTable() {
    return this.state.currentTable.map((wa, idx) => (
      <Table.Row key={idx}>
        ...
        {this.state.button3 && (
          <Table.Cell>
            {new Intl.NumberFormat('ru-RU').format(wa.kol1)}
          </Table.Cell>
        )}
        {this.state.button3 && (
          <Table.Cell>
            {new Intl.NumberFormat('ru-RU').format(wa.summa1)}
          </Table.Cell>
        )}
        {this.state.button3 && (
          <Table.Cell>
            {new Intl.NumberFormat('ru-RU').format(wa.kol2)}
          </Table.Cell>
        )}
        {this.state.button3 && (
          <Table.Cell>
            {new Intl.NumberFormat('ru-RU').format(wa.summa2)}
          </Table.Cell>
        )}
        <Table.Cell>
          {new Intl.NumberFormat('ru-RU').format(wa.kol3)}
        </Table.Cell>
        <Table.Cell>
          {new Intl.NumberFormat('ru-RU').format(wa.summa3)}
        </Table.Cell>
        <Table.Cell>
          {new Intl.NumberFormat('ru-RU').format(wa.kol4)}
        </Table.Cell>
        <Table.Cell>
          {new Intl.NumberFormat('ru-RU').format(wa.summa4)}
        </Table.Cell>
      </Table.Row>
    ));
  }
  ...
  renderTableFooter() {
    return this.state.currentTableTotal.map((wa, idx) => (
      <Table.Row key={idx}>
        ...
        {this.state.button3 && (
          <Table.HeaderCell>
            <b>{new Intl.NumberFormat('ru-RU').format(wa.kol1)}</b>
          </Table.HeaderCell>
        )}
        {this.state.button3 && (
          <Table.HeaderCell>
            <b>{new Intl.NumberFormat('ru-RU').format(wa.summa1)}</b>
          </Table.HeaderCell>
        )}
        {this.state.button3 && (
          <Table.HeaderCell>
            <b>{new Intl.NumberFormat('ru-RU').format(wa.kol2)}</b>
          </Table.HeaderCell>
        )}
        {this.state.button3 && (
          <Table.HeaderCell>
            <b>{new Intl.NumberFormat('ru-RU').format(wa.summa2)}</b>
          </Table.HeaderCell>
        )}
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.kol3)}</b>
        </Table.HeaderCell>
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.summa3)}</b>
        </Table.HeaderCell>
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.kol4)}</b>
        </Table.HeaderCell>
        <Table.HeaderCell>
          <b>{new Intl.NumberFormat('ru-RU').format(wa.summa4)}</b>
        </Table.HeaderCell>
      </Table.Row>
    ));
  }
    ...
    return (
      ...
      >
        ...
        <Grid>
                ...
                <Table.Body>
                  <Table.Row>
                    <Table.Cell>
                      <Button.Group>
                        <Button
                          positive={this.state.button1}
                          onClick={() => this.onSelectTableType(1)}
                        >
                          Параметры поиска
                        </Button>
                        <Button.Or text=" | " />
                        <Button
                          positive={this.state.button2}
                          onClick={() => this.onSelectTableType(2)}
                        >
                          Уборочная система
                        </Button>
                        <Button.Or text=" | " />
                        <Button
                          positive={this.state.button3}
                          onClick={() => this.onSelectTableType(3)}
                        >
                    ...
                    <Table.Row>
                      ...
                      <Table.Cell>
                        ...
                        <DatePicker
                          ...
                          selected={this.state.searchTerm.dateFrom}
                          locale="ru"
                          onChange={event =>
                            this.onInputChange(event, 'dateFrom')
                          }
                        ...
                        <DatePicker
                          ...
                          showYearDropdown
                          dropdownMode="select" // timezone="UTC"
                          selected={this.state.searchTerm.dateTo}
                          locale="ru"
                          onChange={event =>
                            this.onInputChange(event, 'dateTo')
                          }
                          dateFormat="DD.MM.YYYY"
                        />
                      </Table.Cell>
          ...
          {((this.state.button2 && !this.state.button3) ||
            (!this.state.button2 && this.state.button3)) && (
            <Grid.Row>
              <Grid.Column mobile={16} tablet={16} computer={16}>
                <Header as="h3" block>
                ...
                <Table striped celled id="serrep1Table">
                  ...
                  <Table.Footer>{this.renderTableFooter()}</Table.Footer>
                </Table>
              </Grid.Column>
            </Grid.Row>
          )}
