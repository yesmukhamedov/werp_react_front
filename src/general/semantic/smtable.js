...
class Smtable extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.tableParams !== undefined) {
      // console.log(1);
      if (
        nextProps.tableParams.table !== undefined &&
        nextProps.tableParams.table.length !== undefined
      ) {
        ...
        if (nextProps.tableParams.totalPages === undefined || isNewTable) {
          ...
          if (nextProps.tableParams.paginationSize !== undefined) {
            // console.log(3);
            if (
              nextProps.tableParams.table.length >
              nextProps.tableParams.paginationSize
            ) {
              const remainder =
                nextProps.tableParams.table.length %
                nextProps.tableParams.paginationSize;
              totalPages =
                (nextProps.tableParams.table.length - remainder) /
                nextProps.tableParams.paginationSize;
            ...
            // console.log(4);
            if (
              nextProps.tableParams.table.length > this.state.paginationSize
            ) {
              // console.log(5);
              const remainder =
                nextProps.tableParams.table.length % this.state.paginationSize;
        ...
        this.setState({
          totalPages,
          rowSize: nextProps.tableParams.table.length,
        });
      }
    ...
    if (
      nextProps.tableParams !== undefined &&
      nextProps.tableParams.currentPage !== undefined
    ) {
  ...
  renderHeaderRows(rows) {
    return rows.map((row, index) => (
      <Table.Row key={index}>{this.renderHeaderCells(row.columns)}</Table.Row>
    ));
  }
  renderHeader() {
    // console.log(this.props.tableParams)
    if (this.props.tableParams === null || this.props.tableParams === undefined)
      return;
    const headers = this.props.tableParams.headers;
  ...
  renderBodyCells(row) {
    // console.log(row);
    const columns = this.props.tableParams.columns;
    if (columns === null || columns === undefined || columns.length === 0)
      return;

    return columns.map((column, index) => {
      const colname = column.colname;
      let colvalue = '';
  ...
  renderBodyRows(rows) {
    ...
    const paginationSize = this.props.tableParams.paginationSize
      ? this.props.tableParams.paginationSize
      : this.state.paginationSize;
    const currentPage = this.props.tableParams.currentPage
      ? this.props.tableParams.currentPage
    ...
    if (this.props.tableParams.pagination) {
      ...
      if (start + paginationSize > rows.length) {
        ...
        rowLength = start + paginationSize;
      }
    }
    ...
    for (let i = start; i < rowLength; i++) {
      ...
      result.push(
        <Table.Row key={rownum}>{this.renderBodyCells(row)}</Table.Row>,
      );
  ...
  renderBody() {
    if (this.props.tableParams === null || this.props.tableParams === undefined)
      return;
    const table = this.props.tableParams.table;
  ...
  renderFooterRows(rows) {
    return rows.map((row, index) => (
      <Table.Row key={index}>{this.renderFooterCells(row.columns)}</Table.Row>
    ));
  }
  renderFooter() {
    if (this.props.tableParams === null || this.props.tableParams === undefined)
      return;
    const footers = this.props.tableParams.footers;
    if (
      (footers === null || footers === undefined || footers.length === 0) &&
      !this.props.tableParams.pagination
    )
      return;

    let isFooters = false;
    if (!(footers === null || footers === undefined || footers.length === 0)) {
      isFooters = true;
    }

    return (
      <Table.Footer>
        {isFooters && this.renderFooterRows(footers)}
        {this.props.tableParams.pagination && this.renderPagination()}
      </Table.Footer>
    );
  }
  ...
  renderPagination() {
    ...
    if (columns === null || columns === undefined || columns.length === 0)
      return;
    const colspan = this.props.tableParams.columns.length;
    return (
      <Table.Row>
        <Table.HeaderCell colSpan={colspan}>
          <Menu floated="right" pagination>
            ...
            <Menu.Item
              icon
              onClick={() => this.handleItemClick(this.state.currentPage - 1)}
              disabled={this.state.currentPage === 1}
            >
            ...
            <Menu.Item
              icon
              onClick={() => {
                this.handleItemClick(this.state.currentPage + 1);
              }}
            ...
            <Menu.Item
              icon
              onClick={() => this.handleItemClick(this.state.totalPages)}
              disabled={this.state.currentPage === this.state.totalPages}
            >
