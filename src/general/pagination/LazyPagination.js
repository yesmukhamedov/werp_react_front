...
class LazyPagination extends Component {
    ...
    renderItem(buttonsCount) {
        ...
        if(buttonsPerPage >= buttonsCount){
            ...
            const to = Math.min(currentPage+(buttonsPerPage-parseInt(buttonsPerPage/2,10)),buttonsCount);
    ...
    render(){
        ...
        return <Menu floated='right' pagination>
            {currentPage > 0?<Menu.Item as='a' icon onClick={() => this.props.onItemClick(0)}>
                    <Icon name='left chevron' />
                </Menu.Item>:''}
            {this.renderItem(buttonsCount)}
            {(buttonsCount-1) > currentPage?<Menu.Item as='a' icon onClick={() => this.props.onItemClick(buttonsCount-1)}>
