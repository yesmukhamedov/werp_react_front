...
class Frep5 extends Component {
    constructor(props) {
        ...
        this.onFilterChangeReactTable = this.onFilterChangeReactTable.bind(
            this,
        );
    ...
    exportExcel() {
        ...
        excelDownload(
            'core/finance/reports/frep5/downloadExcel',
            'frep5Total.xls',
            'outputTable',
    ...
    renderSearchTab() {
        ...
        const ctOptions = [
            {
                key: 0,
                text: formatMessage(messages.frep5CtStandard),
                value: '0',
            },
            { key: 1, text: formatMessage(messages.frep5CtBu), value: '1' },
            {
                key: 2,
                text: formatMessage(messages.frep5CtPresent),
                value: '2',
            },
            {
                key: 3,
                text: formatMessage(messages.frep5CtService),
                value: '3',
            },
        ];
    ...
    searchFrep5() {
        ...
        if (errors === null || errors === undefined || errors.length === 0) {
            this.props.fetchDynamicFAGM('core/finance/reports/frep5/search', {
                ...this.state.searchTerm,
                branchList: this.state.searchTerm.branchList.join(),
                ctTypeList: this.state.searchTerm.ctTypeList.join(),
                conDateFrom: this.state.searchTerm.conDateFrom.format(
                    'YYYY-MM-DD',
                ),
                        ...
                        outputTable.map((wa, idx) => {
                            return (
                                ...
                                >
                                    ...
                                    <Table.Cell>
                                        {moneyFormat(wa.nalDmbtr)}
                                    </Table.Cell>
                                    <Table.Cell>
                                        {moneyFormat(wa.nalWrbtr)}
                                    </Table.Cell>
                                    <Table.Cell>
                                    ...
                                    <Table.Cell>
                                        {moneyFormat(wa.rasDmbtr)}
                                    </Table.Cell>
                                    <Table.Cell>
                                        {moneyFormat(wa.rasWrbtr)}
                                    </Table.Cell>
                                    <Table.Cell>{wa.totKol}</Table.Cell>
                                    <Table.Cell>
                                        {moneyFormat(wa.totDmbtr)}
                                    </Table.Cell>
                                    <Table.Cell>
                                        {moneyFormat(wa.totWrbtr)}
                                    </Table.Cell>
                                </Table.Row>
                            );
                        })}
    ...
    getDetail(branchId, waers, ps) {
        this.props.modifyLoader(true);
        this.props.fetchDynamicFAGM('core/finance/reports/frep5/searchDetail', {
    ...
    renderDetail() {
        ...
        let t1r1c7 = {
            Header: ({ value }) => (
                <b>{formatMessage(messages.contractDate)}</b>
            ),
            accessor: 'conDate', //Cell: obj => <span>{obj.original.budat}</span>,
            filterMethod: (filter, rows) =>
                matchSorter(rows, filter.value, {
                    keys: ['_original.conDate'],
                }),
            filterAll: true,
            width: 100,
        };
