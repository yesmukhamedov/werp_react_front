...
class Fmcp extends Component {
    ...
    if (stateFieldName === 'zregOrConNum') {
      ...
      if (newVal !== undefined) {
        ...
        if (this.props.contract.psRows) {
          const psRows = JSON.parse(JSON.stringify(contract.psRows));
          let remain = parseFloat(newVal);
          for (let i = 0; i < psRows.length; i++) {
            const due = psRows[i].sum2 - psRows[i].paid;
  ...
  onInputChangePS(value, key) {
    const psRows = JSON.parse(JSON.stringify(this.props.contract.psRows));
    const newVal = moneyInputHanler(value, 2);
  ...
  validate() {
    ...
    if (hkont_d === null || hkont_d === undefined || !hkont_d) {
    ...
    if (price - paid < summa) {
      errors.push(errorTable[`18${language}`]);
    }
    ...
    if (psRows !== null && psRows.length > 0) {
      for (let i = 0; i < psRows.length; i++) {
        ...
        psCurrentpaymentamount =
          parseFloat(psCurrentpaymentamount) +
          parseFloat(waPsRows.currentpaymentamount);
      }
      if (paid !== psPaid) {
        errors.push(errorTable[`19${language}`]);
      }
      if (parseFloat(summa) !== psCurrentpaymentamount) {
        errors.push(errorTable[`19${language}`]);
      }
    }
      ...
      <Segment padded size="small">
        ...
        <Table collapsing>
          <Table.Body>
            ...
            <Table.Row>
              <Table.Cell>{formatMessage(messages.fioClient)}</Table.Cell>
              <Table.Cell>
            ...
            <Table.Row>
              <Table.Cell>{formatMessage(messages.dealer)}</Table.Cell>
              <Table.Cell width="10">
            ...
            <Table.Row>
              <Table.Cell>{formatMessage(messages.finAgent)}</Table.Cell>
              <Table.Cell>
            ...
            <Table.Row>
              <Table.Cell>{formatMessage(messages.waers)}</Table.Cell>
              <Table.Cell>
            ...
            <Table.Row>
              <Table.Cell>{formatMessage(messages.price)}</Table.Cell>
              <Table.Cell>
            ...
            <Table.Row>
              <Table.Cell>{formatMessage(messages.paid)}</Table.Cell>
              <Table.Cell>
            ...
            <Table.Row>
              <Table.Cell>{formatMessage(messages.remainder)}</Table.Cell>
              <Table.Cell>
                <Input
                  fluid
              ...
              <Table.Row key={key}>
                ...
                <Table.Cell>
                  <Input
                    ...
                    onFocus={handleFocus}
                    onChange={(e, { value }) =>
                      this.onInputChangePS(value, key)
                    }
                  />
                </Table.Cell>
              </Table.Row>
  ...
  renderSave() {
    ...
    return (
      <Table collapsing>
        ...
        <Table.Body>
          <Table.Row>
            <Table.Cell>{formatMessage(messages.cashBank)}</Table.Cell>
            ...
            <Table.Cell>
              <Dropdown
                ...
                value={hkont_d}
                onChange={(e, { value }) =>
                  this.onInputChange(value, 'hkont_d')
                }
              />
            </Table.Cell>
          </Table.Row>
          <Table.Row>
            <Table.Cell>{formatMessage(messages.amount)}</Table.Cell>
