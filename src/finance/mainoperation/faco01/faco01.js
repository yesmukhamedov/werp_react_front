...
import {  Container, Header, Segment, List, Button, Icon, Table, Dropdown, Input, Label, Checkbox } from 'semantic-ui-react';
import moment from 'moment';
import FaHeader from '../../faHeader';
import {f4FetchDepartmentList, f4FetchCurrencyList, f4FetchExchangeRateNational} from '../../../reference/f4/f4_action';
import {clearfaBkpf, changefaBkpf, fetchExpenseHkontsByBukrs, fetchCashBankHkontsByBranch, saveFiSrcDocs} from '../../fa_action';
...
class Faco01 extends Component {
    ...
    onInputChange(value,stateFieldName){ 
      ...
      {
        let hkont_s = !this.state.withInvoice?this.state.hkont_s:'';
    ...
    save(){      
      ...
      if (errors===null || errors===undefined || errors.length===0){        
        ...
        let args = {bkpf,amount:this.state.amount,lifnr:this.state.lifnr, hkont_s:this.state.hkont_s, hkont_h:this.state.hkont_h, withInvoice:this.state.withInvoice};
        this.props.saveFiSrcDocs(args,'FACO01',()=>this.initializeBkpfBseg());
      }
      else{
    ...
    validate(){
      ...
      if (lifnr===null || lifnr===undefined || !lifnr) { errors.push(errorTable['9'+language]); }
      if (amount===null || amount===undefined || !amount || parseFloat(amount)<=0) { errors.push(errorTable['61'+language]); }
      if (hkont_h===null || hkont_h===undefined || !hkont_h) { errors.push(errorTable['3'+language]); }
      if (withInvoice===true && (hkont_s===null || hkont_s===undefined || !hkont_s)) { errors.push(errorTable['12'+language]); }
      return errors;
    }
    ...
    render(){
      ...
      const {customerF4ModalOpen, withInvoice, amount, lifnrName, hkont_s, hkont_h}  = this.state;
      ...
      .map((wa,idx)=>{
        ...
        return (
            ...
            <Container fluid style={{ marginTop: '2em', marginBottom: '2em', paddingLeft: '2em', paddingRight: '2em'}}>
                ...
                <CustomerF4Modal open={customerF4ModalOpen} onCloseCustomerF4={(bool)=>this.customerF4ModalOpenHanlder(bool)} 
                ...
                <Label color="red" ribbon>
                    {formatMessage(messages.position)}
                            ...
                            <Table.Header>
                                <Table.Row>
                                    <Table.HeaderCell>{formatMessage(messages.customer)}</Table.HeaderCell>
                                    ...
                                    <Table.HeaderCell colspan="2">{formatMessage(messages.toCreatInvoice)}</Table.HeaderCell>
                                </Table.Row>
                            </Table.Header>
                                <Table.Body>
                                    <Table.Row>
                                        <Table.Cell>
                                            <span> {lifnrName} <Button icon='external' 
                                        ...
                                        <Table.Cell>
                                            ...
                                            onFocus={handleFocus} 
                                            maxLength='18'  onChange={(e, {value}) => this.onInputChange(value,'amount')} />
                                        </Table.Cell>                                        
                                        <Table.Cell>
                                            <Dropdown  search selection options={hkontOptions?hkontOptions:[]} 
                                            value={hkont_h}  onChange={(e, { value }) => this.onInputChange(value,'hkont_h')}/>  
                                        </Table.Cell>
                                        <Table.Cell>
                                          <Checkbox
                                              checked={withInvoice}
                                              onChange={(e, { value }) => this.onInputChange(value,"withInvoice")}
                                              />
                                        </Table.Cell>
                                        <Table.Cell textAlign='left'>
                                            <Dropdown fluid search selection options={this.props.hkontOptions2?this.props.hkontOptions2:[]} 
                                            value={hkont_s}  onChange={(e, { value }) => this.onInputChange(value,'hkont_s')} disabled={!withInvoice}/>  
                                        </Table.Cell>
                                    </Table.Row>                                    
                                </Table.Body>                     
                            </Table>
...
export default connect(mapStateToProps,{ f4FetchDepartmentList, f4FetchCurrencyList, modifyLoader,
  f4FetchExchangeRateNational, changefaBkpf, clearfaBkpf, fetchExpenseHkontsByBukrs, fetchCashBankHkontsByBranch, saveFiSrcDocs }) (injectIntl(Faco01));
