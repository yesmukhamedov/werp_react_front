...
export function fetchItems(id, params) {
    return function(dispatch) {
        ...
        doGet(`core/crm/report/${id}`, params)
            .then(({ data }) => {
...
export function fetchChildItems(id, params) {
    return dispatch => doGet(`core/crm/report/` + id, params);
}

export function fetchMeta(id) {
    return function(dispatch) {
        dispatch(modifyLoader(true));
        doGet(`core/crm/report/meta/${id}`)
            .then(({ data }) => {
                dispatch(modifyLoader(false));
                dispatch({
...
export function updateDirectorNote(demoId, note) {
    return function(dispatch) {
        dispatch(modifyLoader(true));
        doPut(`core/crm/report/note/${demoId}`, { note })
            .then(({ data }) => {
                dispatch(modifyLoader(false));
...
export function fetchDemoRecommender(demoId) {
    return dispatch =>
        doGet(`core/crm/report/find-recommender`, {
            demoId: demoId,
        });
}
