...
export function fetchPhoneNumberHistory(phoneId) {
    return function(dispatch) {
        doGet(`core/call/number-history/${phoneId}`)
            .then(({ data }) => {
                dispatch({
...
export function updateReco(reco) {
    return function(dispatch) {
        doPut(`core/crm/reco/${reco.id}`, { ...reco })
            .then(({ data }) => {
                dispatch({
...
export function fetchRecoCurrentData(type) {
    return function(dispatch) {
        dispatch(modifyLoader(true));
        doGet(`core/crm/reco/current/${type}`)
            .then(({ data }) => {
                let actionType;
                switch (type) {
                    case 'new':
...
export function fetchRecoArchive(params) {
    return function(dispatch) {
        dispatch(modifyLoader(true));
        doGet(`core/crm/reco/archive`, params)
            .then(({ data }) => {
                dispatch(modifyLoader(false));
                dispatch({
...
export function checkPhoneNumber(staffId, phoneNumber) {
    return function(dispatch) {
        ...
        doGet(`core/crm/phone/check/${staffId}/${phoneNumber}`)
            .then(({ data }) => {
                dispatch(modifyLoader(false));
                dispatch({
...
export function fetchReasons(typeId) {
    return function(dispatch) {
        doGet(`core/reference/reasons/${typeId}`)
            .then(({ data }) => {
...
export function createRecoListNew(o) {
    return dispatch => doPost(`core/crm/reco/create`, o);
}

export function blankReco(context, contextId) {
    return dispatch =>
        doGet(`core/crm/reco/create?context=${context}&contextId=${contextId}`);
