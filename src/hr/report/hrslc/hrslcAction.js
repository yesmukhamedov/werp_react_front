...
export const fetchHrslcList = param => {
    return function(dispatch) {
        ...
        doGet(`core/hr/staff/report/hrslc/list`, param)
            .then(({ data }) => {
...
export const fetchStaffList = param => {
    return function(dispatch) {
        dispatch(modifyLoader(true));
        doGet(`core/hr/staff/report/hrslc/list_staff`, param)
            .then(({ data }) => {
                dispatch(modifyLoader(false));
                dispatch({
...
export const fetchWorkStatusList = () => {
    return function(dispatch) {
        dispatch(modifyLoader(true));
        doGet(`core/hr/staff/report/hrslc/mobile_track_steps`)
            .then(({ data }) => {
                dispatch(modifyLoader(false));
                dispatch({
...
export const fetchBusinessProcessList = () => {
    return function(dispatch) {
        dispatch(modifyLoader(true));
        doGet(`core/hr/staff/report/hrslc/mobile_tracked_bps`)
            .then(({ data }) => {
                dispatch(modifyLoader(false));
                dispatch({
