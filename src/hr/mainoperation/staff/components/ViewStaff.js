  ...
  handleTabChange (e, data) {
    ...
    if (data.activeIndex === 2) {
      ...
      if (!this.state.staff || !this.state.staff.hasOwnProperty('staff_id')) {
        ...
        axios.get(`${ROOT_URL}/api/hr/staff/` + this.state.staff.staff_id + `/salaries`, {
          headers: {
            authorization: localStorage.getItem('token')}
        }).then((response) => {
          this.setState({
      ...
      <Grid>
        <Grid.Row columns={2}>
          <Grid.Column width={8}>
            ...
            <Grid celled>
              <Grid.Row>
                <Grid.Column width={5} textAlign={'right'}>Номер уд. личности</Grid.Column>
                <Grid.Column width={7}>{stf.passport_id}</Grid.Column>
              </Grid.Row>

              <Grid.Row>
                <Grid.Column width={5} textAlign={'right'}>Кем выдан</Grid.Column>
                <Grid.Column width={7}>{stf.passport_given_by}</Grid.Column>
              </Grid.Row>
              <Grid.Row>
                <Grid.Column width={5} textAlign={'right'}>Дата выдачи</Grid.Column>
                <Grid.Column width={7}>{this.formatTimestamptToDate(stf.passport_given_date)}</Grid.Column>
              </Grid.Row>
              <Grid.Row>
                <Grid.Column width={5} textAlign={'right'}>Срок действия уд.</Grid.Column>
                <Grid.Column width={7}>{this.formatTimestamptToDate(stf.passport_validity)}</Grid.Column>
              </Grid.Row>
            </Grid>
          </Grid.Column>
          ...
          <Grid.Column width={8}>
            <Grid celled>
              <Grid.Row>
                <Grid.Column width={5} textAlign={'right'}>Номер паспорта</Grid.Column>
                <Grid.Column width={7}>{stf.passport_id}</Grid.Column>
              </Grid.Row>

              <Grid.Row>
                <Grid.Column width={5} textAlign={'right'}>Кем выдан (паспорт)</Grid.Column>
                <Grid.Column width={7}>{stf.passport_given_by}</Grid.Column>
              </Grid.Row>
              <Grid.Row>
                <Grid.Column width={5} textAlign={'right'}>Дата выдачи (паспорт)</Grid.Column>
                <Grid.Column width={7}>{this.formatTimestamptToDate(stf.passport_given_date)}</Grid.Column>
              </Grid.Row>
              <Grid.Row>
                <Grid.Column width={5} textAlign={'right'}>Срок действия (паспорт)</Grid.Column>
                <Grid.Column width={7}>{this.formatTimestamptToDate(stf.passport_validity)}</Grid.Column>
              </Grid.Row>
            </Grid>
          </Grid.Column>
  ...
  renderProfile () {
    ...
    return (
      <Container fluid style={{ marginTop: '2em', marginBottom: '2em', paddingLeft: '2em', paddingRight: '2em'}}>
        ...
        <Divider />
        <Tab panes={panes} menu={{ secondary: true, pointing: true }} onTabChange={this.handleTabChange} />
      </Container>
    )
  }
