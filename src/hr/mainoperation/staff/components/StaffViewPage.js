...
import {
  fetchSingleStaff,
  toggleStaffDataFormModal,
  blankStaffData,
  fetchStaffData,
  setStaffDataForUpdate,
  deleteStaffData,
  downloadFile,
  addUploadedFile,
  deleteFile,
...
class StaffViewPage extends Component {
  ...
  renderContactData() {
    return (
      <StaffContactTable
        addresses={this.props.staffDataList[CONTACT_DATA] || []}
      />
    );
  }
  ...
  renderExpensesData(staffId) {
    return (
      ...
      >
        <Button onClick={this.prepareToCreate} floated={'right'} primary>
          Добавить
        </Button>
        <StaffExpencesTable
  ...
  renderSalaryData(items) {
    return (
      ...
      >
        ...
        <StaffSalariesTable
          handleDelete={this.prepareForDelete}
          handleUpdate={this.prepareForUpdate}
          items={items}
        />
      </Container>
    );
  ...
  renderProfile() {
    ...
    const panes = [
      ...
      {
        menuItem: 'Должности',
        render: () => this.renderSalaryData(staffDataList[SALARY_DATA]),
      },
      ...
      {
        menuItem: 'Образование',
        render: () => this.renderEduData(staffDataList[EDU_DATA] || []),
      },
      {
        menuItem: 'Файлы',
        render: () => this.renderFiles(staffDataList[FILE_DATA] || []),
      },
      //{menuItem:'Доп. данные',render:this.renderMainData},
      { menuItem: 'Баланс', render: this.renderMainData },
      {
        menuItem: 'Склад',
        render: () => this.renderMatnrsData(staffDataList[MATNR_DATA] || []),
      },
    ];
    return (
      <Container
      ...
      >
        ...
        <StaffDataFormHandler activeData={STAFF_DATA[this.state.activeIndex]} />
        <Divider />
        <Tab
