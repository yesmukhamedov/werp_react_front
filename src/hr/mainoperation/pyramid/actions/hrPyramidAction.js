...
export function removeStaffFromEmployee(id, callback) {
    return function(dispatch) {
        dispatch(modifyLoader(true));
        doPut(`core/hr/pyramid/remove-staff/${id}`, {})
            .then(() => {
                dispatch(modifyLoader(false));
                if (callback) {
                    callback();
...
export function blankItem(parentId) {
    return function(dispatch) {
        dispatch(modifyLoader(true));
        doGet(`core/hr/pyramid/blank/${parentId}`)
            .then(({ data }) => {
                dispatch(modifyLoader(false));
                // console.log(data)
                dispatch({
...
export function updatePyramid(p) {
    return dispatch => doPut(`core/hr/pyramid/${p.id}`, p);
}

export function createPyramid(p) {
    return dispatch => doPost(`core/hr/pyramid`, p);
}
