...
class TimesheetPage extends Component {
  ...
  renderActionButtons() {
    ...
    return (
      <div>
        ...
        <Button onClick={this.saveData} primary floated={'right'}>
          {messages['Form.Save']}
        </Button>
      </div>
    );
      ...
      <Form>
        <Form.Group widths="equal">
          ...
          <Form.Select
            ...
            name="year"
            label={messages['Form.Year']}
            options={YEAR_OPTIONS}
            placeholder={messages['Form.Year']}
            onChange={this.handleChange}
          />
          ...
          <Form.Select
            ...
            name="month"
            label={messages['Form.Month']}
            options={monthsArrayToOptions(moment.months())}
            placeholder={messages['Form.Month']}
            onChange={this.handleChange}
          />
        </Form.Group>
        <Form.Button onClick={this.loadItems}>
          {messages['Form.Form']}
        </Form.Button>
      </Form>
    );
  ...
  handleDayStatusChange = (item, dayNumber, value) => {
    ...
    selectedItems[staffId]['days'][dayNumber] = {
      number: dayNumber,
      statusName: value,
      enabled: true,
    };
  ...
  renderDaysCell = item => {
    ...
    let content = filteredDays.map(day => {
      let opValue = staffDays[day.number]
        ? staffDays[day.number]['statusName']
        : day['statusName'] || ''; //(day) @ToDo
      return (
        <Table.Cell
          negative={opValue === 'MISSING'}
          positive={opValue === 'PRESENT'}
          key={day.number}
        >
          <select
            value={opValue}
            onChange={e =>
              this.handleDayStatusChange(item, day.number, e.target.value)
            }
          >
            {statuses.map(s => {
              return (
                <option key={s.name} value={s.name}>
                  {this.statusCodeByLocale(s, locale)}
                </option>
              );
            })}
          </select>
  ...
  renderData() {
    ...
    return (
      ...
      >
        <Segment clearing>{this.renderActionButtons()}</Segment>
        <Divider clearing />
  ...
  renderStatusDescriptions() {
    ...
    return (
      <Message>
        <Message.Header>
          {messages['Hr.Timesheet.StatusDescriptionTitle']}
        </Message.Header>
        <Message.List>
          {statuses.map(status => {
            return (
              <Message.Item key={status.code}>
                <strong>{this.statusCodeByLocale(status, locale)}</strong>-{' '}
                {locale === 'en'
                  ? status.descriptionEn
                  : locale === 'tr'
                  ? status.descriptionTr
                  : status.description}
              </Message.Item>
            );
          })}
