...
export function renderRecoCategoryBtn(categoryId, categoryName) {
  if (categoryName) {
    return (
      <Button
        size="tiny"
        basic
        color={RECO_CATEGORY_COLORS[categoryId] || 'grey'}
      >
        {categoryName}
      </Button>
    );
  ...
  const category = _.find(RECO_CATEGORIES, { key: categoryId });
  return (
    <Button
      size="tiny"
      basic
...
export function renderRecoCategoryAsQty(categoryId, qty) {
  const category = _.find(RECO_CATEGORIES, { key: categoryId });
  const title = category && category.text ? category.text : 'Неизвестно';
  return (
    <Label
      basic
      title={title}
