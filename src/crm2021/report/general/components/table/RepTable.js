...
class RepTable extends Component {
  ...
  renderRep740Modal() {
    ...
    return (
      <Modal size={'small'} open={this.props.repModalOpened}>
        ...
        <Modal.Actions>
          <Button negative onClick={() => this.props.toggleRepModal(false)}>
            Отмена
          </Button>
          <Button
            positive
            icon="checkmark"
            onClick={() => this.props.updateDirectorNote(demoId, directorNote)}
            labelPosition="right"
            content="Сохранить"
          />
        </Modal.Actions>
