    ...
    if (this.state.call.callResultId == CALL_RESULT_REFUSE) {
      ...
      return (
        <Form.Select
          ...
          options={this.props.callRefuseOptions}
  ...
  renderCallForm() {
    return (
      <Form>
        <Form.Group widths="equal">
          <Form.Field
            value={this.state.call.phoneNumber}
            readOnly
            control={Input}
            label="Тел. номер"
        ...
        <Form.Group widths="equal">
          <Form.Field
            ...
            label="Примечание звонка"
            placeholder="Примечание звонка"
          />
          <Form.Field />
        </Form.Group>
        <Divider />
  ...
  render() {
    ...
    return (
      <Modal size="large" open={open} onClose={this.close} onOpen={onOpen}>
        <Modal.Header>Звонок по номеру: 00000000000</Modal.Header>
        <Modal.Content>
