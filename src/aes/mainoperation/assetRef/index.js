...
class AssetRef extends Component {
    ...
    newOs(newOs) {
        this.props.newObject('core/aes/os/save', newOs, 'NEW_OS');
    }
    newType1(newtype1) {
        this.props.newObject('core/aes/type1/save', newtype1, 'NEW_TYPE1');
    }
    newType2(newtype2) {
        this.props.newObject('core/aes/type2/save', newtype2, 'NEW_TYPE2');
    }
    newType3(newType3) {
        this.props.newObject('core/aes/type3/save', newType3, 'NEW_TYPE3');
    }
    newDetail(newDetail) {
        this.props.newObject('core/aes/detail/save', newDetail, 'NEW_DETAIL');
    }
    newRnum(newRnum) {
        this.props.newObject('core/aes/room/save', newRnum, 'NEW_RNUM');
    }
    newStatus(newStatus) {
        this.props.newObject('core/aes/status/save', newStatus, 'NEW_STATUS');
    }
    newCompBr(newCompBr) {
        this.props.newObject('core/aes/compbr/save', newCompBr, 'COMP_BR');
    }

    /****************************find sub items  */

    findType1(os_id) {
        this.props.findObject('core/aes/find/type1/', os_id);
    }

    /******************************************************************* CALL DISABLE FUNCTION */
    disableOs(os) {
        this.props.disableObject('core/aes/os/disable', os, 'DIS_OS');
    }
    disableType1(type1) {
        this.props.disableObject('core/aes/type1/disable', type1, 'DIS_TYPE1');
    }
    disableType2(type2) {
        this.props.disableObject('core/aes/type2/disable', type2, 'DIS_TYPE2');
    }
    disableType3(type3) {
        this.props.disableObject('core/aes/type3/disable', type3, 'DIS_TYPE3');
    }
    disableDetail(det) {
        this.props.disableObject('core/aes/det/disable', det, 'DIS_DET');
    }
    ...
    getDetail() {
        ...
        let out = listDetail.map(detail => {
            return {
                ...
                text: `${detail.detail_name} ${parseInt(
                    detail.detail_code,
                    10,
                )}`,
