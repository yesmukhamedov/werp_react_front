...
export function fetchCCBranch(bukrs, country_id) {
    return function(dispatch) {
        ...
        doPost(`core/aes/cc/fetch`, { bukrs, country_id })
            .then(({ data }) => {
                modifyLoader(false);
                dispatch({
                    type: CCBRANCH_AES,
...
export function newAes(newAes) {
    return function(dispatch) {
        ...
        doPost(`core/aes/aes/save`, newAes)
            .then(({ data }) => {
                dispatch(modifyLoader(false));
                if (data) {
                    dispatch(successed());
...
export function saveApprRej(apprRej, rejected) {
    return function(dispatch) {
        dispatch(modifyLoader(false));
        doPut('core/aes/apprjec', { apprRej, rejected })
            .then(({ data }) => {
                dispatch(modifyLoader(false));
                if (data) {
                    dispatch(successed());
...
export function findCompBrCode(bukrs, branch_id) {
    return function(dispatch) {
        dispatch(modifyLoader(false));
        doPost(`core/aes/cbcode/fetch`, { bukrs, branch_id })
            .then(({ data }) => {
                modifyLoader(false);
                dispatch({
                    type: COMP_BR,
