...
class AccountabilityStaffListPage extends Component {
    ...
    componentWillMount() {
        doGet(`core/reference/companies`)
            .then(response => {
    ...
    handleDropdownChange(fieldName, value) {
        switch (fieldName) {
            ...
            case 'limit':
                const tempCheched = this.state.limitChecked;
                this.setState({
                    ...this.state,
    ...
    loadItems() {
        if (
            !this.state.selectedBukrs ||
            this.state.selectedBukrs.length === 0
        ) {
            this.setState({
                ...this.state,
        ...
        doGet(`core/logistics/report/accountability-staff`, {
        ...
        return (
            <Form>
                <Form.Field>
                    ...
                    <Dropdown
                        error={this.state.errors.bukrsHasError}
                        placeholder="Компания"
                        fluid
                        selection
                        options={this.state.bukrsOptions}
                        onChange={(e, { value }) =>
                            this.handleDropdownChange('company', value)
                        }
                    />
                </Form.Field>
                <Form.Field>
                    ...
                    <Dropdown
                        ...
                        onChange={(e, { value }) =>
                            this.handleDropdownChange('branch', value)
                        }
                    />
                </Form.Field>
                <Form.Field>
                    <Checkbox
                        label="Лимит"
                        checked={this.state.limitChecked}
                        onChange={e => this.handleDropdownChange('limit', '')}
                    />
                </Form.Field>
                <Button onClick={this.loadItems} loading={this.state.loading}>
                    Сформировать
                </Button>
            </Form>
        );
                        ...
                        <Table.Row key={item.staffId}>
                            ...
                            <Table.Cell>
                                <Link
                                    ...
                                    to={`/logistics/report/accountability-staff/${item.staffId}`}
                                >
                                    Детально
                                </Link>
                            </Table.Cell>
