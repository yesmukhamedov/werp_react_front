...
class LeaveReasonListPage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      model: {},
  ...
  loadItems() {
    this.props.fetchLeaveReasons().then(({ data }) => {
      this.setState({
        ...this.state,
        items: data,
      });
  ...
  handleFormSubmit() {
    ...
    if (hasError) {
      ...
      let resp = null;
      if (model.new) {
        resp = this.props.createLeaveReason(model);
      } else {
        resp = this.props.updateLeaveReason(model);
      }
