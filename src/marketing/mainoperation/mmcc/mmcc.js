                ...
                .filter(
                    item =>
                        ...
                        item.parent_branch_id ===
                            waSelectedBranch.parent_branch_id,
                )
                .forEach(item => {
                        ...
                        item =>
                            (item.bukrs == wa.bukrs &&
                                item.business_area_id ==
                                    waSelectedBranch.business_area_id) ||
                            (item.bukrs == wa.bukrs &&
        ...
        if (fieldName === 'price') {
            ...
            setPs(prev => {
                ...
                const idx = prev.findIndex(
                    item => item.paymentScheduleId === id,
                );
        ...
        if (ps && ps.length > 0) {
            setPs(prev => {
                ...
                for (let nextIdx = idx + 1; nextIdx < ps.length; nextIdx++) {
                    addMonth += 1;
                    newArray[nextIdx].paymentDate = moment(
                        lastPaymentDate,
                        'YYYY-MM-DD',
                    )
                        .add(addMonth, 'M')
    ...
    const onSave = event => {
        ...
        if (errors === null || errors === undefined || errors.length === 0) {
            props.onSaveContractMmcc(
                'core/marketing/contract/mmcc/saveContract',
                {
                    contract,
                    ps,
                    contractPromoList,
