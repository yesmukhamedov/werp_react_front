...
class Prcltgs extends Component {
    ...
    constructor(props){
        ...
        this.state={searchTerm:{bukrs:'',branchList:[],selectedCategory:3}, companyOptions:[], branchOptions:[],selectedBranchKey:null, 
        branchList:[],priceList:[],selectedPriceListIndex:null, editPriceListIndex:null, matnrOptions:[], waers:"",countryId:null, loading:false};
    }
    ...
    onInputChange(value,stateFieldName){
        ...
        if (stateFieldName==="bukrs")
        {
            ...
            const newBranchOptions = this.state.branchOptions.filter(item=>item.tovarCategory===value || value===3).map(item2=>{
                    return {
                        key: item2.key,
                        text: item2.text,
                        value: item2.value,
    ...
    onInputChangePrice(value,fieldName,index,obj){
        ...
        let priceList = this.state.priceList.slice();
        ...
        {
            obj.matnr=value;
            this.state.matnrOptions.filter(item=>item.value===value).map(item=>{
    ...
    onInputChangePayTemp(value,fieldName,ptIndex,plIndex,obj){
        if (obj.dataStatus==="del") return;
        let priceList = this.state.priceList.slice();
        let price = priceList[plIndex];
        let ptList = price.ptList;
        ...
        for (var i = 0; i < ptList.length; i++) {
            ...
            price.price = price.price + (ptList[i].monthlyPaymentSum*ptList[i].monthNum);
            if (i===0) {
                price.firstPayment = ptList[i].monthlyPaymentSum;
            }    
            else
            {
                price.month = price.month + ptList[i].monthNum;
            }
        }
    ...
    onSearchPriceList(branchId){
        this.setState({loading:true});
        axios.get(`${ROOT_URL}/api/marketing/prcltgs/search`, {
            headers: {
                authorization: localStorage.getItem('token')
            },
            params:{
    ...
    onSaveClick(){
        axios.post(`${ROOT_URL}/api/marketing/prcltgs/save`, {
        ...
        .then((response) => {
            ...
            let branchId = this.state.selectedBranchKey;
    ...
    onSelectBranch(key){
        this.setState({selectedBranchKey:key,selectedPriceListIndex:null,editPriceListIndex:null, 
            matnrOptions:[], waers:"", countryId:null});
    ...
    editPrice(index){
        var priceList = this.state.priceList.slice();  
    ...
    removePayTemp(index,payTempRow){
        ...
        if (this.state.priceList[this.state.selectedPriceListIndex].dataStatus==="del") return;
        if(index===0) return;
        let priceList = this.state.priceList;
        let price = priceList[this.state.selectedPriceListIndex];
        let ptList = price.ptList.slice();
    ...
    addNewPayTemp(){
        ...
        let price = priceList[this.state.selectedPriceListIndex];
        let ptList = price.ptList;
        let arraySize = ptList.length;
        let nextPtOrder = ptList[arraySize-1].ptOrder+1;
        let ptRow = {
                    info:"",
                    monthNum:1,
                    monthlyPaymentSum:0,
        ...
        price.month = price.month + ptRow.monthNum;
        price.price = price.price + (ptRow.monthlyPaymentSum*ptRow.monthNum);
        price.remain = price.remain + (ptRow.monthlyPaymentSum*ptRow.monthNum);
        ...
        this.setState({priceList,editPriceListIndex:this.state.selectedPriceListIndex});
    }
    ...
    removePrice(index,obj){
        ...
        priceList[index].ptList.map(item=>{item.dataStatus="del"});
    ...
    addNewPrice(){
        ...
        var newArray = this.state.priceList.slice();    
        newArray.push(obj);
               
        this.setState({priceList:newArray,editPriceListIndex:newArray.length-1,selectedPriceListIndex:newArray.length-1});
        

    }

    displayIconStatus(dataStatus){
        if (dataStatus==="new") return <Icon name='checkmark'  size='large' color="green"/>;
        if (dataStatus==="del") return <Icon name='minus circle'  size='large' color="red"/>;
        if (dataStatus==="mod") return <Icon name='options'  size='large' color="orange"/>;
    ...
    renderTableBranchList() {
        return this.state.branchList.map((wa,idx)=>{
            ...
            return (
                ...
                <Table.Row active={wa.key===this.state.selectedBranchKey} key={wa.key} onClick={()  => this.onSelectBranch(wa.key)}>                         
    ...
    renderTablePriceListHeaders() {
        return this.state.priceList.map((wa,idx)=>{
            // console.log(wa);
            return (
                <Table.Row active={idx===this.state.selectedPriceListIndex} key={idx} onClick={()  => this.onSelectPriceList(wa.priceListId,idx)}>
                    
                    <Table.Cell className="clickable" >{this.displayIconStatus(wa.dataStatus)}</Table.Cell>   
                    ...
                    <Table.Cell className={this.makeClickable(idx)} >
                        ...
                        <Dropdown  selection options={this.state.matnrOptions} 
                            value={wa.matnr}  text={wa.matnrName}
                             onChange={(e, { value }) => this.onInputChangePrice(value,'matnr',idx,wa)}
                            />}
                    </Table.Cell>                     
                    <Table.Cell className="clickable">{new Intl.NumberFormat('ru-RU').format(wa.price)} {wa.waers}</Table.Cell>
                    <Table.Cell className="clickable">{new Intl.NumberFormat('ru-RU').format(wa.firstPayment)}</Table.Cell>
                    <Table.Cell className="clickable">{new Intl.NumberFormat('ru-RU').format(wa.remain)}</Table.Cell>
                    ...
                    <Table.Cell className="clickable">
                        ...
                        {idx===this.state.editPriceListIndex && 
                            <Dropdown compact selection options={premOptions} 
                            value={wa.premDiv}  
                            onChange={(e, { value }) => this.onInputChangePrice(value,'premDiv',idx,wa)}
                            /> 
                    ...
                    <Table.Cell> <Icon name='edit'  size='large'  className="clickableIcon" onClick={(event)=>this.editPrice(idx)}/>
                    <Icon name='remove'  size='large'  className="clickableIcon" color="red" onClick={(event)=>this.removePrice(idx,wa)}/>
                    </Table.Cell>
                </Table.Row> 
            );
    ...
    renderTablePriceListRows() {
        ...
        if (this.state.priceList[this.state.selectedPriceListIndex].ptList===null) return;
        return this.state.priceList[this.state.selectedPriceListIndex].ptList.map((wa_pt,idx)=>{
            if (wa_pt.ptOrder===0) {
                month = "Перв. взнос";
            }
            else {month = wa_pt.monthNum;}
            return (<Table.Row  key={idx} >
                        <Table.Cell>{(wa_pt.ptOrder===0 || this.state.selectedPriceListIndex!==this.state.editPriceListIndex) && month}
                            {wa_pt.ptOrder!==0 && this.state.selectedPriceListIndex===this.state.editPriceListIndex && 
                            <Dropdown  compact selection options={monthNumOptions} 
                            value={wa_pt.monthNum}  onChange={(e, { value }) => this.onInputChangePayTemp(value,'monthNum',idx,this.state.editPriceListIndex,wa_pt)}
                            /> 
                            
                            }
                        </Table.Cell>
                        <Table.Cell>
                            {this.state.selectedPriceListIndex!==this.state.editPriceListIndex && wa_pt.monthlyPaymentSum}
                            {this.state.selectedPriceListIndex===this.state.editPriceListIndex &&
                            
                            <Input value={wa_pt.monthlyPaymentSum} 
                            onChange={(e, { value }) => this.onInputChangePayTemp(value,'monthlyPaymentSum',idx,this.state.editPriceListIndex,wa_pt)}
                             className="moneyInput" />}
                             
                        </Table.Cell>
                        <Table.Cell>
            {idx!==0 && <Icon name='remove'  size='large'  className="clickableIcon" color="red" onClick={(event)=>this.removePayTemp(idx,wa_pt)}/>}
            </Table.Cell>
            </Table.Row> );
        })
                ...
                <Grid textAlign='justified' >
                    <Grid.Row  columns={1}>
                        <Grid.Column mobile={16} tablet={16} computer={16}>
                            <Button icon labelPosition='left' primary size='small' disabled={this.state.selectedBranchKey===null} onClick={()=> this.onSaveClick()}>
                ...
                <Grid textAlign='justified' >
                    <Grid.Row  columns={3}>
                        <Grid.Column mobile={16} tablet={16} computer={3}>
                            <Dropdown placeholder='Компания' fluid selection options={this.state.companyOptions} 
                            value={this.state.searchTerm.bukrs}  onChange={(e, { value }) => this.onInputChange(value,'bukrs')} />                            
                        </Grid.Column>
                        <Grid.Column mobile={16} tablet={16} computer={3}> 
                            <Dropdown fluid selection options={categoryOptions} value={this.state.searchTerm.selectedCategory} 
                            onChange={(e, { value }) => this.onInputChange(value,'category')} />
                        </Grid.Column>
                        <Grid.Column mobile={16} tablet={16} computer={1}> 
                            {this.state.waers!=="" && <Label as='a' color='blue' >{this.state.waers}</Label>}
                            ...
                            <Table compact striped celled selectable id="priceListHeaders">
                                ...
                                <Table.Footer>
                                    ...
                                    <Table.HeaderCell></Table.HeaderCell>
                                        ...
                                        <Table.HeaderCell></Table.HeaderCell>
                                        <Table.HeaderCell></Table.HeaderCell>
                                        <Table.HeaderCell><Icon name='add'  size='large'  className="clickableIcon" color="green" onClick={(event)=>this.addNewPrice()}/></Table.HeaderCell>
                                    </Table.Row>
                                </Table.Footer>   
                            </Table>  
                        </Grid.Column>
                            ...
                            <Table compact striped celled  id="priceListRows">
                                ...
                                <Table.Footer>
                                    <Table.Row>
                                        <Table.HeaderCell></Table.HeaderCell>
                                        <Table.HeaderCell></Table.HeaderCell>
                                        <Table.HeaderCell><Icon name='add'  size='large'  className="clickableIcon" color="green" onClick={(event)=>this.addNewPayTemp()}/></Table.HeaderCell>
                                    </Table.Row>
                                </Table.Footer>
